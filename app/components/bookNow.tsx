...
const BookNow = () => {
    ...
    // Boat pricing based on hours (from your requirements)
    ...
    useEffect(() => {
        ...
        if (boatRentalCount > 0 && pricingType === "Hourly") {
            const boatCost = BOAT_HOURLY_PRICING[hourlyDurationBoat] || BOAT_HOURLY_PRICING[8];
            cost += boatRentalCount * boatCost;
        } else if (boatRentalCount > 0 && pricingType === "Half Day") {
            cost += boatRentalCount * 350; // Half day fixed price
        } else if (boatRentalCount > 0 && pricingType === "Full Day") {
            cost += boatRentalCount * 850; // Full day fixed price - CORRECT, $650 from table
        }

        // Jet Ski pricing
        if (jetSkisCount > 0 && pricingType === "Hourly") {
            cost += jetSkisCount * JET_SKI_HOURLY_COST * hourlyDurationJetSki;
        } else if (jetSkisCount > 0 && pricingType === "Half Day") {
            cost += jetSkisCount * (JET_SKI_HOURLY_COST * 4);
        } else if (jetSkisCount > 0 && pricingType === "Full Day") {
            cost += jetSkisCount * (JET_SKI_HOURLY_COST * 8);
        }

        // Waiting time
        if (waitingTime > 0) {
            cost += waitingTime * WAITING_TIME_COST_PER_HOUR;
        }

        // Water sports
        waterSport.forEach((sport) => {
            const participants = sportPeople[sport] || 0;
            cost += participants * WATER_SPORT_COSTS[sport];
        });
        ...
        <div id="book" className="text-[#003b73] p-6 mt-6 mb-4">
            ...
            <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="col-span-2 bg-[#d4f1f4] bg-opacity-20 border-1 shadow-lg px-4 md:px-6 py-6 rounded-lg space-y-6">
                                    ...
                                    >
                                        {[1, 2, 3, 4, 5, 6, 7, 8].map((hour) => (
                                            <option key={hour} value={hour}>
                                                {hour} hour{hour > 1 ? 's' : ''} - ${BOAT_HOURLY_PRICING[hour]}
                                            </option>
                                        ))}
                                    </select>
                                </div>
                            ...
                            {jetSkisCount > 0 && (
                                <div className="flex-1">
                                    ...
                                    >
                                        {[1, 2, 3, 4, 5, 6, 7, 8].map((hour) => (
                                            <option key={hour} value={hour}>
                                                {hour} hour{hour > 1 ? 's' : ''} - ${hour * JET_SKI_HOURLY_COST} per jet ski
                                            </option>
                                        ))}
                                    </select>
                                </div>
                                        ...
                                        <label key={sport} className="flex items-center justify-between py-1 mb-2 px-2 hover:bg-gray-100 cursor-pointer">
                                            <div className="flex items-center space-x-2">
                                                <input
                                                    ...
                                                    className="h-4 w-4"
                                                />
                                                <span className="text-sm">{sport} (${WATER_SPORT_COSTS[sport as WaterSport]})</span>
                                            </div>
                                            <input
                                                type="number"
                                                min="1"
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="flex-1">
                            <label className="block text-sm font-medium text-black mb-1">Waiting Time (Hours)</label>
                            <input
                                type="number"
                                min="0"
                                max="6"
                                ...
                                className="p-2 border rounded-md w-full"
                                value={waitingTime}
                                onChange={(e) => setWaitingTime(Math.min(parseFloat(e.target.value), 6))}
                            />
                        </div>
                            ...
                            {boatRentalCount > 0 && (
                                <div className="pb-3 border-b border-gray-200">
                                    ...
                                    <p className="flex justify-between font-semibold text-gray-900 mt-2">
                                        <span>Cost:</span>
                                        <span>
                                            ${pricingType === "Hourly" ? BOAT_HOURLY_PRICING[hourlyDurationBoat] :
                                                pricingType === "Half Day" ? 350 :
                                                    650}
                                        </span>
                                    </p>
                                </div>
                            ...
                            {jetSkisCount > 0 && (
                                <div className="pb-3 border-b border-gray-200">
                                    ...
                                    <p className="flex justify-between font-semibold text-gray-900 mt-2">
                                        ...
                                        <span>
                                            ${jetSkisCount * (
                                                pricingType === "Hourly" ? JET_SKI_HOURLY_COST * hourlyDurationJetSki :
                                                    pricingType === "Half Day" ? JET_SKI_HOURLY_COST * 4 :
                                                        JET_SKI_HOURLY_COST * 8
                                            )}
                                        </span>
                                    </p>
                                    <p className="text-xs text-gray-500 mt-1">
                            ...
                            {waitingTime > 0 && (
                                <div className="pb-3 border-b border-gray-200">
                                    <p className="flex justify-between font-medium text-gray-800">
                                        ...
                                        <span>{waitingTime} hour{waitingTime > 1 ? 's' : ''}</span>
                                    </p>
                                    <p className="flex justify-between font-semibold text-gray-900 mt-2">
                                        <span>Cost:</span>
                                        <span>${waitingTime * WAITING_TIME_COST_PER_HOUR}</span>
                                    </p>
                                    <p className="text-xs text-gray-500 mt-1">
                                        ${WAITING_TIME_COST_PER_HOUR}/hour
                                    </p>
                                </div>
                            )}
                                    ...
                                    {waterSport.map((sport) => (
                                        <div key={sport} className="mb-2 last:mb-0">
                                            <p className="flex justify-between text-sm">
                                                <span>{sport} (x{sportPeople[sport] || 1})</span>
                                                <span>${(sportPeople[sport] || 1) * WATER_SPORT_COSTS[sport]}</span>
                                            </p>
                                            <p className="text-xs text-gray-500 text-right">
                                                ${WATER_SPORT_COSTS[sport]} per person
                                            </p>
                                        </div>
                                    ))}
                                </div>
